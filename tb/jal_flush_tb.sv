...
module jal_flush_tb;
    ...
    always_comb begin
        case (instruction_address)
            ...
            32'h08: instruction_data = 32'h00C0_00EF; // jal  x1, +12 -> 0x14
    ...
    initial begin
        ...
        if (debug_a0 !== 32'd7)
            $fatal(1, "JAL target instruction did not execute: a0 = %0d", debug_a0);
        if (dut.rf.registers[1] !== 32'd12)
            $fatal(1, "JAL link value is wrong: x1 = %0d", dut.rf.registers[1]);
        if (dut.rf.registers[11] !== 32'd1)
            $fatal(1, "JAL wrong path wrote x11 = %0d", dut.rf.registers[11]);
        if (dut.rf.registers[12] !== 32'd2)
            $fatal(1, "JAL wrong path wrote x12 = %0d", dut.rf.registers[12]);

        $display("JAL FLUSH PASS");
        $finish;
    end
